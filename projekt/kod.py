...
def draw_obstacles(obstacles):
    for obstacle in obstacles:
        pygame.draw.rect(game_window, dark_gray, pygame.Rect(obstacle[0], obstacle[1], 20, 20))

def generate_obstacle(snake_body, fruit_position):
    while True:
        obstacle_position = [random.randrange(3, (window_x // 10) - 3) * 10,
                             random.randrange(3, (window_y // 10) - 3) * 10]
        # Zkontrolujeme, zda se překážka nepřekrývá s hadem ani ovocem
        if obstacle_position not in snake_body and obstacle_position != fruit_position:
...
def main_menu():
    ...
    while True:
        game_window.fill(dark_gray)  # Vymazání obrazovky černou barvou

        # Vykreslíme text do středu obrazovky
        draw_text("SNAKE GAME", font, yellow, window_x // 2, window_y // 4)
        draw_text("Press SPACE to Play", font, white, window_x // 2, window_y // 2)

        pygame.display.update()  # Aktualizace zobrazení
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    return  # Ukončení hlavního menu a přechod do hry


# Přidání funkcionality pro zlaté jablko i překážky do hlavní smyčky
def game_loop():
    ...
    while True:
        ...
        while True:
            ...
            if snake_position == fruit_position:
                ...
                if score % 50 == 0:
                    obstacles.append(generate_obstacle(snake_body, fruit_position))
            else:
                snake_body.pop()
            ...
            # Generování zlatého jablka
            if not golden_fruit_spawn and random.randint(1, 1000) <= 10:
            ...
            # Kontrola vypršení zlatého jablka
            ...
            # Reset rychlosti po uplynutí efektu zlatého jablka
            if golden_fruit_timer and time.time() - golden_fruit_timer > golden_fruit_duration:
